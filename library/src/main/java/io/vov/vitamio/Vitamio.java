...
public class Vitamio {
  ...
  public static boolean isInitialized(Context ctx) {
    ...
    File path1 = new File(ctx.getApplicationInfo().nativeLibraryDir, "libstlport_shared.so");
    if (path1.exists()) {
      vitamioLibraryPath = ctx.getApplicationInfo().nativeLibraryDir + "/";
    } else {
      vitamioLibraryPath = ctx.getApplicationInfo().dataDir + "/";
    }
//    String dataPath = ctx.getApplicationInfo().dataDir;
//    String libDir = ctx.getApplicationInfo().nativeLibraryDir.substring(ctx.getApplicationInfo().nativeLibraryDir.indexOf("lib/"));
//    vitamioLibraryPath = ctx.getApplicationInfo().dataDir + "/lib/";
    System.out.println("Vitamio Lib Path: " + vitamioLibraryPath);
    return true;
  }
